...
export default function BacSiLayout() {
  ...
  useEffect(() => {
    ...
    if (role !== 'BAC_SI' && role !== 'BACSI' && role !== 'BS') {
    ...
    if (email) {
      nhanVienService.getByMaTaiKhoan(email).then((data) => {
        if (data) {
          ...
          setNhanVien({ hoVaTen: 'Tài khoản Bác sĩ', maNV: '---' });
        }
      });
    }
  }, [navigate]);
    ...
    <div className="w-full min-h-screen flex bg-[#F3F4F6] font-sans antialiased">
      ...
      <aside className="w-64 min-h-screen bg-white border-r border-slate-200 flex flex-col shadow-sm shrink-0">
        ...
        <div className="flex items-center gap-3 px-5 py-5 border-b border-slate-100">
          ...
          <div>
            <p className="text-[11px] font-black uppercase text-emerald-700 tracking-wider leading-tight">
              Hệ thống Hiến máu
            </p>
        ...
        <div className="px-4 py-4 border-b border-slate-100">
          <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
            <div className="w-10 h-10 rounded-full bg-emerald-600 flex items-center justify-center text-white text-sm font-black shrink-0 shadow">
              {initials}
            </div>
        ...
        <div className="px-3 py-4 border-t border-slate-100">
          <button
            ...
            className="flex items-center gap-3 px-3 py-2.5 w-full rounded-xl text-sm font-semibold text-slate-500 hover:bg-red-50 hover:text-red-600 transition-all group"
          >
        ...
        <header className="h-16 bg-white border-b border-slate-200 px-8 flex items-center justify-between shrink-0 shadow-sm z-10">
          ...
          <div className="relative w-80">
            ...
            <input
              ...
              className="w-full h-10 bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 text-sm text-slate-700 placeholder-slate-400 outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/10 transition-all"
            />
          </div>
